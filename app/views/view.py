...
class UsuarioAPI(MethodView):
    ...
    def get(self, user_id=None):
        ...
        if user is None:
            return (
                jsonify(
                    USER_NOT_FOUND=f"user with id {user_id} not found"
                ),
                404,
            )
...
app.add_url_rule(
    "/user/<user_id>", view_func=UsuarioAPI.as_view("usuario_id")
)
...
class PublicacionAPI(MethodView):
    ...
    def get(self, publicacion_id=None):
        ...
        if user is None:
            resultado = (
                f"publication with id {publicacion_id} not found"
            )
    ...
    def post(self):
        ...
        try:
            publicacion_json = publicacionBasicSchema().load(
                request.json
            )
        ...
        return (
            jsonify(
                AGREGADO=publicacionBasicSchema().dump(
                    publicacion_json
                )
            ),
            201,
    ...
    def delete(self, publicacion_id):
        try:
            publicacion = Publicacion.query.filter_by(
                id=publicacion_id
            ).one()
        except:
            return (
                jsonify(
                    ERROR=f"publication with id {publicacion_id} not found"
                ),
                404,
            )
        ...
        db.session.commit()
        return jsonify(
            PUBLICACION_ELIMINADA=comentarioBasicSchema().dump(
                publicacion
            )
        )
...
app.add_url_rule(
    "/publicaciones",
    view_func=PublicacionAPI.as_view("publicaciones"),
)
app.add_url_rule(
    "/publicaciones/<publicacion_id>",
    view_func=PublicacionAPI.as_view("publicacion_id"),
)
...
class TemaAPI(MethodView):
    ...
    def get(self, tema_id=None):
        ...
        if tema is None:
            return (
                jsonify(ERROR=f"theme with id {tema_id} not found"),
                404,
            )
    ...
    def delete(self, tema_id):
        ...
        try:
            ...
            return (
                jsonify(ERROR=f"theme whit id {tema_id} not found"),
                404,
            )
...
app.add_url_rule(
    "/temas/<tema_id>", view_func=TemaAPI.as_view("tema_id")
)
...
class ComentarioAPI(MethodView):
    def get(self, comentario_id=None):
        if comentario_id is None:
            ...
            resultado = comentarioSchema().dump(
        ...
        if comentario is None:
            return (
                jsonify(
                    COMENTARY_NOT_FOUND=f"comentary with id {comentario_id} not found"
                ),
                404,
            )
    ...
    def post(self):
        try:
            comentario_json = comentarioBasicSchema().load(
                request.json
            )
        ...
        return (
            jsonify(
                AGREGADO=comentarioSchema().dump(comentario_json)
            ),
            201,
        )
    ...
    def delete(self, comentario_id):
        try:
            ...
            return (
                jsonify(
                    ERROR=f"comentary with id {comentario_id} not found"
                ),
                404,
            )

        return jsonify(
            ELIMINADO=comentarioBasicSchema().dump(comentario)
        )


app.add_url_rule(
    "/comentarios", view_func=ComentarioAPI.as_view("comentarios")
)
app.add_url_rule(
    "/comentarios/<comentario_id>",
    view_func=ComentarioAPI.as_view("comentario_id"),
)
